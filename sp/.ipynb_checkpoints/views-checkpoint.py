...
def home(request):
    if request.method == 'POST':
        ...
        target = forms.get("target", tomorrow)
        if not target:
            target = tomorrow
        ...
        if user:
            ...
            if baseline:
                ...
                output = ""
                if dataset.lower() == 'vaccine':
                    output = "{} is not a valid Twitter account or has not made a Covid-19 vaccine related Tweet since {}.".format(user, from_date)
                else:
                    output = "{} is not a valid Twitter account or has not made a mask related Tweet since {}.".format(user, from_date)
                return render(request, 'home.html', {"results": "Results", "output": output})
        else:
